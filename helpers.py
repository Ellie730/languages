...
def update():
    ...
    try:
        ...
        db.execute("""UPDATE users SET time = ?, finnish_ns = ?, french_ns = ?, german_ns = ?, italian_ns = ?, spanish_ns = ? WHERE id = ?""", 
                (session["datetime"], session["Finnish"]["review_count"], session["French"]["review_count"], session["German"]["review_count"], session["Italian"]["review_count"], session["Spanish"]["review_count"], session["user_id"]))
