...
def my_deck():
    ...
    db.execute ("""SELECT COUNT (*) from words
    ...
    AND alternates.alternate = user_progress.alternate""", (session["deck_id"], session["user_id"]))
    pages = int((int(db.fetchall()[0][0]) - 1)/50)
    db.execute ("SELECT card_order FROM users WHERE id = ?", (session["user_id"],))
    order = db.fetchall()[0][0]
    db.execute (f""" SELECT * from words
    JOIN user_progress ON words.id = user_progress.word_id
    JOIN alternates ON alternates.original = words.id
    WHERE id IN (SELECT word_id FROM deck_contents WHERE deck_id = ?) AND user_id = ?
    AND alternates.alternate = user_progress.alternate
    ORDER BY {order} LIMIT ?, 50""", (session["deck_id"], session["user_id"], 50*page))
    cards = db.fetchall()
...
def search_decks ():
    ...
    if request.method == "POST":
        ...
        db.execute("""SELECT * FROM decks WHERE deck_id LIKE ? OR language LIKE ? OR name LIKE ? 
                   OR medium LIKE ? OR genre LIKE ? OR author LIKE ? OR date LIKE ?"""
